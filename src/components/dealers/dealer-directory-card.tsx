...
export type DirectoryDealer = {
  ...
  /** Up to three thumbnails from the newest photographed stock. */
 ...
 *
 * A strip of three photographs from the dealership's newest stock leads the card, so the directory
 * reads as showrooms rather than a phone book. They are decorative (alt="", inside the card whose
 * name is the link), lazy, and the 320px thumbnail rendition. A dealership with no photographed
 * stock simply has no strip.
 */
  ...
  return (
    <article className="rn-card">
      {photos.length > 0 ? (
        <div
          aria-hidden="true"
          className={`grid gap-0.5 overflow-hidden rounded-t-[calc(var(--rn-radius-md)-1px)] bg-line ${
            photos.length === 1
              ? "grid-cols-1"
              : photos.length === 2
                ? "grid-cols-2"
        ...
        >
          {photos.map((photo) => (
            <img
              ...
              loading="lazy"
              decoding="async"
              className={`w-full bg-subtle object-cover ${photos.length === 1 ? "aspect-[3/1]" : "aspect-[4/3]"}`}
            />
          ))}
        </div>
      ) : null}
            ...
            {dealer.minPrice !== null && dealer.maxPrice !== null ? (
              <p className="text-body tabular">
                ...
                {dealer.minPrice === dealer.maxPrice ? (
                  <span className="whitespace-nowrap">{formatRand(dealer.minPrice)}</span>
                ) : (
                  <>
                    <span className="whitespace-nowrap">{formatRand(dealer.minPrice)}</span> to{" "}
                    <span className="whitespace-nowrap">{formatRand(dealer.maxPrice)}</span>
                  </>
                )}
              </p>
            ) : (
